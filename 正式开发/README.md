...
## 目录结构与状态（M1+M2+M3 + P0 四新域 + 对外网关已落地，2026-07-21）

| 目录 | 内容 | 状态 |
|---|---|---|
| [mcp-services/](mcp-services/) | **八域** FastMCP 业务服务（调度/航线/预检/飞行 + 空域/告警/媒体/排期；真实平台直连、多租户 key 鉴权、Nacos 注册、瘦身返回、hydrate 缓存、编辑器 REST） | ✅ 单测 + 端到端冒烟 + 网关冒烟 16 项（现网实测） |
...
| [eval/](eval/) | **61 条**评测集（41-61 覆盖四新域）+ Gateway 跑批 runner（自动清理平台测试航线） | 全量跑批结果见 eval/last_run.json |

## 本地全链路启动（M2/M3 实测口径）

```bash
# 1. 八域 MCP（8201-8204、8206-8209；.env 里 MCP_SERVICE_IP 必须是当前 en0 IP，见排障①）
...
与演示版**代码完全独立**（不 import 根目录任何模块）；八域共享单进程状态，
端口 8201-8204、8206-8209，审批 8205（与演示版 8101+ 错开）。
