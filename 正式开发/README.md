...
# 2.5 通用前端 UI 服务（8213，docs/08：确认卡片页/视图页，任意宿主沙盒可开）
cd uav_extensions && UAV_MCP_API_KEY=... APPROVAL_BASE=http://127.0.0.1:8205 \
  ../deerflow/backend/.venv/bin/python -m uav_extensions.ui_service
...
| [docs/08-通用前端与MCP-UI方案.md](docs/08-通用前端与MCP-UI方案.md) | 业务 UI 通用化：view_url 页面（确认卡片/视图落图）+ MCP Apps 路线 |
| [docs/09-用户与身份设计.md](docs/09-用户与身份设计.md) | 身份追责到人：一条身份链×三关注入，声明式(阶段1)→OAuth 2.1(阶段2) |
| [deploy/启动手册.md](deploy/启动手册.md) | **系统启动手册**：A 核心对话链路 / B 完整链路（+前端+对外网关），逐服务命令+验证+排障 |
| [deploy/docker/README.md](deploy/docker/README.md) | **Docker 部署**：核心三件套（mcp/审批/UI）生产 Linux compose，镜像+网络模型+验证 |
